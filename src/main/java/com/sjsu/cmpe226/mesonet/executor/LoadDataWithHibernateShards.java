 ...
 * 
 */
public class LoadDataWithHibernateShards {
	...
	public void readFromOutDataFileMethod(String filename) {
		try {
			...
			System.out.println("Total size of the file to read "+ fipstream.available());

			int count = 0;
			in = new DataInputStream(fipstream);
			br = new BufferedReader(new InputStreamReader(in));
			String strLine = null;
			WeatherDataShardDao weatherDataDao = new WeatherDataShardDao();
			...
			System.out.println("Read the end of file");
			in.close();
